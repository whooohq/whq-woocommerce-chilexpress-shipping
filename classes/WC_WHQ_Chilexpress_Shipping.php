	...
	if (! class_exists('WC_WHQ_Chilexpress_Shipping')) {
		class WC_WHQ_Chilexpress_Shipping extends WC_Shipping_Method
		{
			...
			public function __construct( $instance_id = 0 )
			{
				...
				$this->soap_or_rest           = $this->get_option('soap_or_rest');
				$this->rest_api_key_cobertura = $this->get_option('rest_api_key_cobertura');
				$this->rest_api_key_tarificacion_QA = $this->get_option('rest_api_key_tarificacion_qa');
				$this->rest_api_key_tarificacion_PROD = $this->get_option('rest_api_key_tarificacion_prod');
			...
			public function init_form_fields()
			{
				$this->form_fields = array(
					...
					'shipments_types_names' => array(
						...
						'type'        => 'text',
						'description' => __('<strong>Lista separada por comas</strong>. Permite cambiar los textos por defecto de Chilexpress para los tipos de envío/servicios: <i>Ultra Rápido, Overnight, Prioritario, Express, Extendido, Extremos</i>. <strong>En ese orden, e incluyendo todos los servicios</strong> (aunque no los utilices). Dejar en blanco para usar los valores que Chilexpress devuelve por defecto.<br/>Ejemplos:<br/><i>Rápido,Noche,Día Siguiente,Día subsiguiente,Durante la Semana,Extremos</i><br/><i>1,2,3,4,5,6</i>', 'whq-wcchp'),
						'default'     => '',
					),
			 ...
			 * @return array        Array containing the cities, with their code as the key
			 */
			public static function get_cities_static( $type = 1 ) {
				global $whq_wcchp_default;
			...
			private function get_states()
			{
				...
				if (false === $regions || substr($regions,0,5) == 'Error') {
					// Retrieve the hard-coded ones
					...
					if ($this->is_soap()){
						...
						foreach ($regions as $region) {
							$regions_array["$region->regionId"] = $region->regionName;
						}
					}
					...
					if ( ! empty( $this->shipments_types_names ) ) {
						...
						if ( count( $renames ) == 7 || count( $renames ) == 5 ) {
							...
							elseif (stripos($values[1], 'sin servicio') !== false) {
								// Name needed for cart/checkout detection. Don't allow change.
